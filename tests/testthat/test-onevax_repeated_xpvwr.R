...
test_that("run_onevax_repeated_xpvwr works correctly, with hesgroups == 1", {
  tt <- seq(0, 5)
  gp <- gono_params(1:2)
  y1 <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1e3)[[1]]
  ...
  y2 <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1e10, vbe = 1)
  ...
  expect_error(y <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1e3, vbe = 1,
                                      primary_uptake = uptake))

  # check can restart
  init_params <- lapply(y2, restart_params)
  y3 <- run_onevax_repeated_xpvwr(seq(max(tt), length.out = 2, by = 1),
                          gp, init_params,
  ...
  y_h4 <- run_onevax_repeated_xpvwr(tt, gp, vea = 0.5, dur_v = 1, vbe = 0.8, hes = 0,
  ...
  ##comment out pn tests, not used in this analysis
...
#  r2_p <- c(0.4)
 # booster_uptake <- c(0.3)
  ...
  expect_error(run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = c(0, 1e2, 1e3),
                                 strategy = "VbE", r1 = 1))
  # check length of dur_p must be 1
  expect_error(run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = 1e3,
                                 dur_p = c(0, 1e2, 1e3),
                                 strategy = "VbE", r1 = 1))
  ## test revax is working
  y6e <- run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = 1e-10, dur_revax = 1e10,
                           strategy = "VoD(L)+VoA(H)",
                           r1 = r1, r2 = r2, r2_p = r2_p,
                           booster_uptake = booster_uptake)
  ...
  y_r1r2_only <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1,
                                   r1 = 0.8, r2 = 0.5,
                                   strategy = "VoD(L)+VoA(H)")

  y_r1r2_boost <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1,
                                    r1 = 0.8, r2 = 0.5,
                                    booster_uptake = 0.4,
                                    strategy = "VoD(L)+VoA(H)")
  ...
  y11 <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 0, strategy = "VoD")
  ...
  y12 <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 1, strategy = "VoD")

  for (i in seq_along(y12)) {

    expect_equal(sum(y12[[i]]$N[, , c(2)]), 0)
  ...
  y13 <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 0.5, strategy = "VoD")
  ...
  y14 <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 0.5, vea_p = 1,
                           strategy = "VoD")
  ...
  y_vea_only <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 0.5, vea = 0.5,
                                  strategy = "VoD")
  y_vea_veap <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 0.5, vea = 0.5,
  ...
  y_no_incid <- run_onevax_repeated_xpvwr(tt, gp, r1 = 1, r2 = 0.5, vea = 1)
  ...
  expect_error(initial_params_repeated_xpvwr(pars[[1]], coverage_p = 0.5,
                                     coverage_v = 0.6))
  ...
  y15 <- run_onevax_repeated_xpvwr(tt, gp, r1 = 0.5, r2 = 0.5, dur_v = 1e90,
                           strategy = "VoD")
  ...
  pars <- lapply(gp[1], model_params)
  ip <- lapply(pars, initial_params_repeated_xpvwr, coverage_p = 0.99999999999999,
               t = 5)

  y16 <- run_onevax_repeated_xpvwr(tt, gp, init_params = ip, dur_p = 1e-90)
  ...
  for (i in seq_along(y16)) {
    ...
    expect_equal(rowSums(y16[[i]]$N[, , 3:5]), rep(0, 6))
  }
  ...
  ip <- lapply(pars, initial_params_repeated_xpvwr, coverage_p = 0.9,
               t = 5)
  y_p <- run_onevax_repeated_xpvwr(tt, gp, init_params = ip, r2_p = 1, dur_v = 1e100,
                           strategy = "VoD(L)+VoA(H)")
  ...
  ip <- lapply(pars, initial_params_repeated_xpvwr, coverage_v = 0.99999999999999,
               t = 5)
  y17 <- run_onevax_repeated_xpvwr(tt, gp, init_params = ip, dur_v = 1e-90)
  ...
  for (i in seq_along(y17)) {
    ...
    expect_equal(rowSums(y17[[i]]$N[, , 5]), rep(0, 6))
  }
...
test_that("run_onevax_repeated_xpvwr works correctly, with hesgroups > 1", {
  ...
  # check 100% vbe vaccinates all new entrants
  y2 <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1e10, vbe = 1, hesgroups = hesgroups)
  
  for (h in 1:hesgroups){
  # cum_vaccinated = 12000 each year = number of entrants
    expect_equal(diff(rowSums(y2[[1]]$cum_vaccinated[, , h])), rep(12e3/hesgroups,
                                                                  max(tt)))
    
    # this also means there should be people moving into 'V'
    expect_true(sum(y2[[1]]$N[, , 2*hesgroups + h]) > 0)
    
  }
  
  # but not P
  expect_equal(sum(y2[[1]]$N[, , hesgroups  + seq_len(hesgroups)]), 0)
    
  # other compartments empty
  expect_equal(sum(y2[[1]]$N[, , -c(seq_len(hesgroups), 2*hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))]), 0)
  
  # and no-one else
  expect_equal(sum(y2[[1]]$cum_vaccinated[, , (hesgroups+1):(5*hesgroups)]), 0)
  expect_equal(sum(y2[[1]]$N[, , -c(seq_len(hesgroups), 2*hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))]), 0)
  ...
  for (i in seq_along(y2))  {
    ...
    ## check each hesitancy subset is the same size
    
    for (h in 1:hesgroups){
    expect_equal(rowSums(y2[[i]]$N[, , h + hesgroups*(0:4) ]), rep(6e+05/hesgroups, max(tt) + 1))
  ...
  ## note this is still an error but not for the below reason! primary_uptake is no longer an argument
  
  #uptake <- c(0, 2.5, 0.5, 0.75, 1)
  #expect_error(y <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1e3, vbe = 1,
  #                                            primary_uptake = uptake, hesgroups = hesgroups))
  ...
  y3 <- run_onevax_repeated_xpvwr(seq(max(tt), length.out = 2, by = 1),
                                  gp, init_params,
                                  vea = 0, dur_v = 1e3, vbe = 1, hesgroups = hesgroups)
  for (i in seq_along(y2)) {
    expect_equal(y2[[i]]$U[length(tt), , ], y3[[i]]$U[1, , ])
    expect_equal(y2[[i]]$I[length(tt), , ], y3[[i]]$I[1, , ])
    expect_equal(y2[[i]]$A[length(tt), , ], y3[[i]]$A[1, , ])
  ...
  expect_error(run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = 1e3, strategy = "VoD", hesgroups = hesgroups,
                                         r1 = list(rep(1, hesgroups)), r2 = list(rep(1, hesgroups-1)), r2_p = list(rep(1, hesgroups)), booster_uptake = list(rep(1, hesgroups))))
  ...
  # check length of vea must be 1
  expect_error(run_onevax_repeated_xpvwr(tt, gp, vea = c(0, 1, 1), dur_v = 1e3, hesgroups = hesgroups,
                                         strategy = "VoD", r1 = list(rep(1, hesgroups))))
  # check length of dur_v must be 1
  expect_error(run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = c(0, 1e2, 1e3), hesgroups = hesgroups,
                                         strategy = "VoD", r1 = list(rep(1, hesgroups))))
  # check length of dur_p must be 1
  expect_error(run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = 1e3, hesgroups = hesgroups,
                                         dur_p = c(0, 1e2, 1e3),
                                         strategy = "VoD", r1 = list(rep(1, hesgroups))))
  
  
  ## test revax is working
  y6e <- run_onevax_repeated_xpvwr(tt, gp, vea = 1, dur_v = 1e-10, dur_revax = 1e10, hesgroups = hesgroups,
                                   strategy = "VoD(L)+VoA(H)",
                                   r1 = r1, r2 = r2, r2_p = r2_p,
                                   booster_uptake = booster_uptake)
  for (i in seq_along(y6e)) {
    ## L who are treated in X, P, or W are offered vaccination
    expect_equal(y6e[[i]]$cum_offered[, 1, c(seq_len(hesgroups), hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))],
                 y6e[[i]]$cum_treated[, 1, c(seq_len(hesgroups), hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))])
    ## H who are treated or screened in X, P, or W are offered vaccination
    expect_equal(y6e[[i]]$cum_offered[, 2, c(seq_len(hesgroups), hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))],
                 y6e[[i]]$cum_treated[, 2,  c(seq_len(hesgroups), hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))] +
                   y6e[[i]]$cum_screened[, 2,  c(seq_len(hesgroups), hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))])
    # and no-one else
    expect_equal(sum(y6e[[i]]$cum_offered[, , -c(seq_len(hesgroups), hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))]), 0)
    
    # uptake % of offered are vaccinated
    
    for (h in 1:hesgroups){
    expect_equal(y6e[[i]]$cum_offered[, , h] * r1[[i]][h],
                 y6e[[i]]$cum_vaccinated[, , h])
    expect_equal(y6e[[i]]$cum_offered[, , hesgroups + h] * r2_p[[i]][h],
                 y6e[[i]]$cum_vaccinated[, , hesgroups + h])
    expect_equal(y6e[[i]]$cum_offered[, , 3*hesgroups + h] * booster_uptake[[i]][h],
                 y6e[[i]]$cum_vaccinated[, , 3*hesgroups + h])
    }
    
    # stratum V empties immediately
    expect_equal(sum(y6e[[i]]$N[, , 2*hesgroups + seq_len(hesgroups)]), 0, tolerance = 1e-5)
    
    # efficacy is perfect
    expect_equal(sum(y6e[[i]]$cum_incid[, , c(hesgroups + seq_len(hesgroups), 2*hesgroups + seq_len(hesgroups), 4*hesgroups + seq_len(hesgroups))]), 0)
    expect_true(all(y6e[[i]]$cum_incid[-1, , c(seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups))] > 0))
  }
  
  # check booster_uptake defaults to r1r2 primary uptake
  y_r1r2_only <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1, hesgroups = hesgroups,
                                           r1 = list(rep(0.8, hesgroups)), r2 =list(rep(0.5, hesgroups)),
                                           strategy = "VoD(L)+VoA(H)")
  
  y_r1r2_boost <- run_onevax_repeated_xpvwr(tt, gp, vea = 0, dur_v = 1, hesgroups = hesgroups,
                                            r1 = list(rep(0.8, hesgroups)), r2 = list(rep(0.5, hesgroups)),
                                            booster_uptake = list(rep(0.4, hesgroups)),
                                            strategy = "VoD(L)+VoA(H)")
  
  expect_equal(y_r1r2_only, y_r1r2_boost)
  
  # When everyone receives one dose (r2 = 0), V, W R all empty
  y11 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list(rep(1, hesgroups)), r2 = list(rep(0, hesgroups)), hesgroups = hesgroups, strategy = "VoD")
  
  for (i in seq_along(y11)) {
    
    expect_equal(sum(y11[[i]]$N[, , -c(seq_len(hesgroups), hesgroups + seq_len(hesgroups))]), 0)
    expect_true(all(rowSums(y11[[i]]$N[, , seq_len(hesgroups)]) > 0))
    expect_true(all(rowSums(y11[[i]]$N[-1, , hesgroups + seq_len(hesgroups)]) > 0))
  ...
  y12 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list(rep(1, hesgroups)), r2 = list(rep(1, hesgroups)), hesgroups = hesgroups, strategy = "VoD")
  
  for (i in seq_along(y12)) {
    
    expect_equal(sum(y12[[i]]$N[, , hesgroups + seq_len(hesgroups)]), 0)
    expect_true(all(rowSums(y12[[i]]$N[, , seq_len(hesgroups)]) > 0))
    expect_true(all(rowSums(y12[[i]]$N[-1, , c(2*hesgroups + seq_len(hesgroups), 3*hesgroups + seq_len(hesgroups), 4*hesgroups + seq_len(hesgroups) )]) > 0))
  ...
  y13 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (1:hesgroups)/(hesgroups)), r2 = list(rep(0.5, hesgroups)), hesgroups = hesgroups, strategy = "VoD")
  
    for (i in seq_along(y13)) {
      for (h in 1:hesgroups){
      expect_equal(y13[[i]]$N[, , hesgroups+h], y13[[i]]$N[, , 2*hesgroups+h])
      }
    }
    # incidence should also be the same as all vex_p default to vex
    ...
    for (i in seq_along(y13)) {
      for (h in 1:hesgroups){
      expect_equal(y13[[i]]$cum_incid[, , hesgroups +h], y13[[i]]$cum_incid[, , 2*hesgroups+h])
      }
    }
  
  
  # test vex_p work independently of vex
  y14 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (1:hesgroups)/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea_p = 1, hesgroups = hesgroups,
                                   strategy = "VoD")
  ...
  for (i in seq_along(y14)) {
    expect_true(all(rowSums(y14[[i]]$N[-1, , hesgroups + seq_len(hesgroups)]) > 0))
    expect_true(all(rowSums(y14[[i]]$N[-1, , 2*hesgroups + seq_len(hesgroups)]) > 0))
    expect_true(sum(y14[[i]]$cum_incid[-1, , hesgroups + seq_len(hesgroups)]) == 0)
    expect_true(all(rowSums(y14[[i]]$cum_incid[-1, , 2*hesgroups + seq_len(hesgroups)]) > 0))
  }
  ...
  y_vea_only <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (1:hesgroups)/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea = 0.5, hesgroups = hesgroups,
                                          strategy = "VoD")
  y_vea_veap <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (1:hesgroups)/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea = 0.5, hesgroups = hesgroups,
                                          vea_p = 0.5, strategy = "VoD")
  ...
  ## note this isn't testing what it thinks it is, as nobody is vaccinated!
  
  #previously this
  #y_no_incid <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (1:hesgroups)/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea = 1, hesgroups = hesgroups)
  
  #but have to add in a strategy
  y_no_incid <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (1:hesgroups)/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea = 1, hesgroups = hesgroups, strategy = "VoD")
  
  
  for (i in seq_along(y_no_incid)) {
      
    expect_true(all(y_no_incid[[i]]$N[-1,,] > 0)) # added in to address previous fault in test
    expect_equal(sum(y_no_incid[[i]]$cum_incid[, , -c((1:hesgroups), 3*hesgroups + seq_len(hesgroups))]), 0)
  }
  ...
  pars <- lapply(gp[1], model_params)
  init_1 <- initial_params_repeated_xpvwr(pars[[1]],
                                          coverage_p = rep(1,hesgroups)/(3*hesgroups), coverage_v = rep(1,hesgroups)/(3*hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups)
  
  
  # expect same number starting in P as in V
  expect_equal(init_1$U0[, hesgroups + seq_len(hesgroups)], init_1$U0[, 2*hesgroups + seq_len(hesgroups)])
  
  init_2 <- initial_params_repeated_xpvwr(pars[[1]],
                                          coverage_p = 2*rep(1,hesgroups)/(9*hesgroups), coverage_v = rep(1,hesgroups)/(9*hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups)
  
  # expect double starting in P as in V
  expect_equal((init_2$U0[, hesgroups + seq_len(hesgroups)]), init_2$U0[, 2*hesgroups + seq_len(hesgroups)] * 2)
  
  # expect error if sum of coverages is greater than groupsizes
  
  expect_error(initial_params_repeated_xpvwr(pars[[1]], coverage_p = 2*rep(1,hesgroups)/(3*hesgroups),
                                             coverage_v = rep(1,hesgroups)/(2*hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups))
  
  
  # expect error if sum of groupsizes is greater than one 
  
  expect_error(initial_params_repeated_xpvwr(pars[[1]], coverage_p = 2*rep(1,hesgroups)/(3*hesgroups),
                                             coverage_v = rep(1,hesgroups)/(2*hesgroups), groupsize = 7*rep(1,hesgroups)/(6*hesgroups), hesgroups = hesgroups))
  
  
  # expect error if sum of coverages is 100% of groupsize
  # (if total coverage is 100%, no asymptomatic infection is seeded)
  
  expect_error(initial_params_repeated_xpvwr(pars[[1]], coverage_p = rep(1,hesgroups)/(2*hesgroups),
                                             coverage_v = rep(1,hesgroups)/(2*hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups))
  ...
  y15 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list(rep(0.5, hesgroups)), r2 =list(rep(0.5, hesgroups)), dur_v = 1e90, hesgroups = hesgroups,
                                   strategy = "VoD")
  
  # number getting vaccinated from X (timepoint 2 as all is 0 in timepoint 1)
  cum_vac <- colSums(y15[[1]]$cum_vaccinated[2, , seq_len(hesgroups)])
  cum_off <- colSums(y15[[1]]$cum_offered[2, , seq_len(hesgroups)])
  
  # 50% of individuals offered will accept and be vaccinated (0.25 + 0.25)
  prop_accept <- cum_vac / cum_off
  expect_true( all(prop_accept == 0.5 ))
  
  # 50% of those treated, are vaccinated
  colSums(y15[[1]]$cum_vaccinated[2, , seq_len(hesgroups)])  / colSums(y15[[1]]$cum_treated[2, , seq_len(hesgroups)])
  
  # number total who received vaccination
  # i.e number in P and V at timepoint 2 given no waning
  p <- colSums(y15[[1]]$N[2, , hesgroups + seq_len(hesgroups)])
  v <- colSums(y15[[1]]$N[2, , hesgroups + seq_len(hesgroups)])
  ...
  pars <- lapply(gp[1], model_params)
  ip <- lapply(pars, initial_params_repeated_xpvwr, coverage_p = 0.99999999999999*rep(1,hesgroups)/(hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups,
               t = 5)
  
  y16 <- run_onevax_repeated_xpvwr(tt, gp, init_params = ip, dur_p = 1e-90, hesgroups = hesgroups)
  
  # entire population starts in P then wanes to X and stays there
  for (i in seq_along(y16)) {
    expect_equal(sum(y16[[i]]$N[1, , hesgroups + seq_len(hesgroups)]), 6e+05)
    expect_equal(rowSums(y16[[i]]$N[2:(max(tt) + 1), , seq_len(hesgroups)]),
                 rep(6e+05, max(tt)))
    
    # V W R are empty for all time
    expect_equal(rowSums(y16[[i]]$N[, , 2*hesgroups + seq_len(3*hesgroups)]), rep(0, 6))
  }
  ...
  ip <- lapply(pars, initial_params_repeated_xpvwr, coverage_p = 0.9*rep(1,hesgroups)/(hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups,
               t = 5)
  y_p <- run_onevax_repeated_xpvwr(tt, gp, init_params = ip, r2_p = list(rep(1, hesgroups)), dur_v = 1e100, hesgroups = hesgroups,
                                   strategy = "VoD(L)+VoA(H)")
  ...
  expect_equal(sum(y_p[[i]]$N[1, , hesgroups + seq_len(hesgroups)]), 6e+05 * 0.9)
  expect_equal(sum(y_p[[i]]$cum_vaccinated[, , - (hesgroups + seq_len(hesgroups))]), 0)
  expect_true(unique(rowSums(y_p[[i]]$cum_vaccinated[-1, , (hesgroups + seq_len(hesgroups))]) > 0))
  expect_true(unique(rowSums(y_p[[i]]$N[-1, , (2*hesgroups + seq_len(hesgroups))]) > 0))
  
  # test individuals still wane to W
  ip <- lapply(pars, initial_params_repeated_xpvwr, coverage_v = 0.99999999999999*rep(1,hesgroups)/(hesgroups), groupsize = rep(1,hesgroups)/(hesgroups), hesgroups = hesgroups,
               t = 5)
  y17 <- run_onevax_repeated_xpvwr(tt, gp, init_params = ip, dur_v = 1e-90, hesgroups = hesgroups)
  ...
  for (i in seq_along(y17)) {
    expect_equal(sum(y17[[i]]$N[1, , 2*hesgroups + seq_len(hesgroups)]), 6e+05)
    expect_equal(rowSums(y17[[i]]$N[2:(max(tt) + 1), , 2*hesgroups + seq_len(hesgroups)]), rep(0, max(tt)))
    
    for (j in 2:max(tt) + 1) {
      expect_true(sum(y17[[i]]$N[j, , 3*hesgroups + seq_len(hesgroups)]) > 0)
    }
    
    # R are empty for all time
    expect_equal(rowSums(y17[[i]]$N[, , 4*hesgroups + seq_len(hesgroups)]), rep(0, 6))
  }
  
  
  
  ### test no vaccination to hesitancy groups with r1 = 0
  #but have to add in a strategy
  y_no_incid_2 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list( (0: (hesgroups-1))/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea = 1, hesgroups = hesgroups, strategy = "VoD")
  for (i in seq_along(y_no_incid_2)) {
    expect_true(all(y_no_incid_2[[i]]$N[, , ((1:4)*hesgroups + 1)] == 0)) # vaccination strata = 0 in 1st hesgroup because totally hesitant
  }
  
  ### test no full or vaccination to hesitancy groups with r2 = 0
  y_no_incid_3 <- run_onevax_repeated_xpvwr(tt, gp, r1 = list(rep(0.5, hesgroups)), r2 = list( (0: (hesgroups-1))/(hesgroups)),  vea = 1, hesgroups = hesgroups, strategy = "VoD")
  for (i in seq_along(y_no_incid_3)) {
    expect_true(all(y_no_incid_3[[i]]$N[, , ((2:4)*hesgroups + 1)] == 0)) # V,W, R strata = 0 in first hesgroup because totally hesitant to second vaccination
  }
  ...
  gp_assort <- c(gp[1], gp[1])
  gp_assort[[2]]$epsilon_hes = 1
  ...
  expect_equal(sum(y_assort[[1]]$cum_incid), sum(y_assort[[2]]$cum_incid), tolerance = 1e-6)
  ...
  expect_true( sum(y_assort_2[[2]]$cum_incid) > sum(y_assort_2[[1]]$cum_incid) )
  for (h in seq_len(hesgroups - 1)){
    expect_true( sum(y_assort_2[[2]]$cum_incid[,, (0:4)*hesgroups + h+1 ]) < sum(y_assort_2[[2]]$cum_incid[,, (0:4)*hesgroups + h ]) )
    }
  
  
  ## no infections to group with no initial infections if assortativity by hesitancy
  init_params <- lapply(y_assort, restart_params)
  
  init_params[[1]]$I0[, hesgroups*(0:(hesgroups-1)) + 1] = 0
  init_params[[1]]$A0[, hesgroups*(0:(hesgroups-1)) + 1] = 0
  init_params[[1]]$S0[, hesgroups*(0:(hesgroups-1)) + 1] = 0
  init_params[[2]]$I0[, hesgroups*(0:(hesgroups-1)) + 1] = 0
  init_params[[2]]$A0[, hesgroups*(0:(hesgroups-1)) + 1] = 0
  init_params[[2]]$S0[, hesgroups*(0:(hesgroups-1)) + 1] = 0
  
  y_assort_3 <- run_onevax_repeated_xpvwr(tt, gp_assort, init_params, r1 = list( (0: (hesgroups-1))/(hesgroups)), r2 = list(rep(0.5, hesgroups)), vea = 0, hesgroups = hesgroups, strategy = "VoD")
  
  expect_true(sum(y_assort_3[[1]]$cum_incid[,, (0:4)*hesgroups + 1 ]) > 0 )
  expect_true(sum(y_assort_3[[2]]$cum_incid[,, (0:4)*hesgroups + 1 ]) == 0 )
...
#test_that("same as xpvwrh if epsilon_hes = ", {
#  
#  
#})
...
## comment this out for the moment - to figure out whether we need to extend to make it compatible with n_erlang > 1 
# 
